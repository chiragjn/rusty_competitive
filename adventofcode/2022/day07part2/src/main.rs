...
fn solve(lines: Box<dyn Iterator<Item = String>>) -> u64 {
    ...
    let mut dir_sizes = file_tree.computed_dir_sizes();
    dir_sizes.sort();
    let free = 70000000 - dir_sizes.last().expect("File tree is empty!");
    *dir_sizes
        .iter()
        .find(|&size| free + size >= 30000000)
        .expect("No directory is large enough to free up space")
}
...
7214296 k"#;
        ...
        assert_eq!(solve(Box::new(it)), 24933642);
    }
}
