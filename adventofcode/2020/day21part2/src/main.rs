...
fn solve(lines: Box<dyn Iterator<Item = String>>) -> String {
    ...
    let mut ingredient2allergent = resolve_mappings(&mut possible_mappings).unwrap();
    let mut pairs: Vec<(String, String)> = ingredient2allergent.drain().collect();
    pairs.sort_by(|(_, first_allergent), (_, second_allergent)| {
        first_allergent.cmp(&second_allergent)
    });
    let dangerous_ingredients: Vec<String> =
        pairs.drain(..).map(|(ingredient, _)| ingredient).collect();
...
sqjhc mxmxvkd sbzzf (contains fish)"#;
        ...
        assert_eq!(solve(Box::new(it)), String::from("mxmxvkd,sqjhc,fvjkl"));
    }
}
